class StringsManager {
  static const String appTitle = 'Quran ';
  static const String ahadithName = 'Ahadith ';
  static const String chapterName = 'Chapter Name';
  static const String versesNum = 'Verses Number ';
  static const String quranLabel = 'quran ';
  static const String hadithLabel = 'hadith';
  static const String tasbehLabel = 'Tasbeh';
  static const String radioLabel = 'Radio';
  static const String settingsLabel = 'Settings';
  static const String textBody = "عدد التسبيحات";
  static const String numTasbihat = "Tasbihat";
  static const String textTasbih = 'سبحان اللة';
}
